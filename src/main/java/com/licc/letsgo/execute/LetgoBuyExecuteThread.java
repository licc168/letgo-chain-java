 ...
 * @see
 */
public class LetgoBuyExecuteThread implements Runnable {
    ...
    public LetgoBuyExecuteThread( RuleBuy ruleBuy, LetsgoService letsgoService, User user, int threadName) {
        this.user = user;
    ...
    public void run() {
        try {
            while (true) {
                PetsOnSale sale = Const.saleQueue.takeFirst();
                Rule rule = new Rule();
